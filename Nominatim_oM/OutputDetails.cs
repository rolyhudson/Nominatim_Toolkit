...
namespace BH.oM.Adapter.Nominatim
{
    ...
    {
        [Description("Include a breakdown of the address into elements. Default is true.")]
        public virtual bool AddressDetails { get; set; } = true;

        [Description("Include additional information in the result if available, e.g. wikipedia link, opening hours. Default is true.")]
        public virtual bool ExtraTags { get; set; } = true;

        [Description("Include a list of alternative names in the results. These may include language variants, references, operator and brand. Default is true.")]
