...
namespace BH.Engine.Adapter.Nominatim
{
    public static partial class Convert
    {
        ...
        public static FeatureCollection ToFeatureCollection(CustomObject customObject)
        {
            ...
            {
                Base.Compute.RecordError("Cannot convert a null object.");
                return null;
            }
            try
            {
                if (customObject.CustomData.ContainsKey("features"))
                {
                    ...
                    Base.Compute.RecordError("No features found in the customObject.");

            }
            catch
            {
                Base.Compute.RecordError("Unexpected data format in customObject check OutputFormat is GeoJSON.");
                return null;
            }
        ...
        private static List<object> GetList(object coordinates)
        {
            ...
            if (coords == null)
            {
                Base.Compute.RecordError("Coordinates could not be converted.");
                return null;
            }
            return coords;
        }
        ...
        private static Point GetPoint(object coordinates)
        {
            ...
            if (coords == null)
                return null;
            if (coords.Count > 2)
                Base.Compute.RecordWarning("Point coordinates contains more than 2 values only the first 2 will be used.");
            if (coords.Count < 2)
            {
                Base.Compute.RecordError("Insufficient coordinates to create a GeoJSON point.");
                return null;
            }
            double lat = 0;
            double lon = 0;
            ...
            {
                Base.Compute.RecordError("Point coordinates could not be converted to doubles.");
                return null;
            }
